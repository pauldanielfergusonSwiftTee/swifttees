...
export default function TournamentSetupPage() {
 ...
 function updateHandicap(player: string, value: number | "") {
  ...
  function roundComplete(roundNumber: number) {
    ...
    const allGroupsComplete = roundSetup.groups.every((group) =>
      group.every((player) => player !== "")
    );

    return Boolean(roundSetup.course) && allGroupsComplete;
  }

  function roundSummary(roundNumber: number) {
    const roundSetup = setup.rounds[roundNumber];
    const groupCount = roundSetup.groups.filter((group) =>
      group.every((player) => player !== "")
    ).length;
    ...
    return `${roundSetup.course} • ${groupCount}/3 groups complete • ${ld} • ${np}`;
  }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <section className="rounded-3xl bg-white border border-slate-200 shadow-sm p-4 md:p-5 mb-4">
          <SectionHeader
            ...
            subtitle="Set the handicap used for this weekend."
                      ...
                      {[1, 2, 3].map((groupNumber, groupIndex) => (
                        ...
                        >
                          ...
                          <input
                            ...
                            className="w-full rounded-xl border border-slate-300 px-3 py-2 font-bold mb-2"
                          />
                          ...
                          <div className="space-y-2">
                            {[1, 2, 3, 4].map((slot, slotIndex) => (
                              <select
                                key={slot}
                                value={roundSetup.groups[groupIndex][slotIndex]}
                                onChange={(e) =>
                                  updateGroupPlayer(
                                    round.round,
                                    groupIndex,
