...
type LiveStats = {
  ...
  bonusWinners: number;
  longestDriveWinners: number;
  nearestPinWinners: number;
};
...
function getGapText(index: number, teams: TeamStanding[]) {
  if (index === 0) {
    const gap = teams[0]?.points - teams[1]?.points;
    return gap > 0 ? `Lead +${gap}` : "Tied lead";
  }

  const gap = teams[0]?.points - teams[index]?.points;
  return gap > 0 ? `-${gap}` : "Tied";
}
...
function buildLiveStory(
  ...
  if (playerGap <= 1 && second) {
    return `⚔️ ${leader.name} leads ${second.name} by just ${playerGap || 0} point. This is right on the edge.`;
  }

  if (teamGap <= 2 && secondTeam) {
  ...
  return `👑 ${leader.name} leads on ${leader.points} points. ${teamLeader?.team ?? "The leading team"} are currently top of the team race.`;
}
...
function buildBattleCards(
    ...
    cards.push({
      ...
      text:
        closestBattle.gap === 0
          ...
          : `Just ${closestBattle.gap} point${closestBattle.gap === 1 ? "" : "s"} separates them.`,
    });
  }
  ...
  if (leader && second) {
    cards.push({
      ...
      label: "One to Watch",
      title: second.name,
      text: `${second.name} is ${leader.points - second.points} behind ${leader.name}.`,
    });
  }
...
function buildMoments(
  ...
  if (leader && second && leader.points - second.points <= 1) {
    moments.push({
      icon: "⚔️",
      title: "Battle Alert",
      text: `${leader.name} and ${second.name} are separated by just ${leader.points - second.points} point.`,
      rarity: "rare",
    });
  }
  ...
  if (topTeam && secondTeam) {
    ...
    if (gap <= 2) {
      moments.push({
        ...
        text: `${topTeam.team} lead ${secondTeam.team} by only ${gap} point${gap === 1 ? "" : "s"}.`,
        rarity: "rare",
      });
    }
  }
  ...
  bonusWinners
    ...
    .forEach((bonus: any) => {
      ...
      moments.push({
        icon: "🎯",
        title: bonusType,
        text: `${bonus.winner_player_name} wins ${bonusType}${bonus.hole ? ` on hole ${bonus.hole}` : ""}.`,
        rarity: "rare",
      });
    });

  if (leader?.through >= 18) {
    moments.push({
      icon: "🏁",
      title: "Round Complete",
      text: `${leader.name} has finished the round on ${leader.points} points.`,
      rarity: "major",
    });
  }
...
export default function LiveCentrePage() {
  ...
  async function loadLeaderboard() {
    ...
    const sortedTeams = Object.values(teams)
      ...
      .map((team, index, allTeams) => ({
        ...
        gapText: getGapText(index, allTeams),
      }));

    const longestDriveWinners = bonusWinners.filter((bonus: any) =>
      String(bonus.bonus_type ?? "").toLowerCase().includes("longest")
    ).length;

    const nearestPinWinners = bonusWinners.filter((bonus: any) => {
      const type = String(bonus.bonus_type ?? "").toLowerCase();
      return type.includes("nearest") || type.includes("closest");
    }).length;

    setLeaderboard(rowsWithPositions);
    setTeamStandings(sortedTeams);
    setLiveStory(buildLiveStory(rowsWithPositions, sortedTeams));
    setBattleCards(buildBattleCards(rowsWithPositions, sortedTeams));
    setMoments(buildMoments(rowsWithPositions, sortedTeams, bonusWinners));
    setLiveStats({
      scoresEntered: stablefordScores.length + scrambleScores.length,
      playersFinished: rowsWithPositions.filter((player) => player.through >= 18)
        .length,
      bonusWinners: bonusWinners.length,
      longestDriveWinners,
      nearestPinWinners,
    });
  }
  ...
  useEffect(() => {
    const channel = supabase
      .channel("live-centre-v1-realtime")
      .on(
        "postgres_changes",
        {
          event: "*",
    ...
    <PageContainer className="bg-slate-100 text-slate-900">
      <section className="rounded-3xl bg-green-950 p-5 text-white shadow-lg">
        ...
        <p className="mt-2 text-sm font-semibold text-green-100">
          Live story, battles, rare moments and WhatsApp-ready updates.
        </p>
      </section>
      ...
      <section className="mt-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="space-y-2">
          {leaderboard.map((player) => {
            const leader = leaderboard[0];
            const gapToLeader = leader ? leader.points - player.points : 0;
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span
                        className={`h-3 w-3 rounded-full ${teamDot(
                          player.team
                        )}`}
                      />
